...
# ---------------- parameters ----------------
NEWTON_MAX = 4          # Newton iterations per implicit solve
MAX_ORDER  = 5          # up to BDF‑5
MIN_FACTOR = 0.2        # step‑size shrink cap
MAX_FACTOR = 5.0        # step‑size growth cap (was 3.0)
...
class BDFTorch:
    def __init__(self, fun:Callable[[float,Tensor],Tensor], t0:float, y0:Tensor,
        ...
        self.n   = y0.numel()
        # initial h from f0
        self.h   = first_step or 0.1/max(1e-4, _norm(fun(t0, y0)))  # 10× larger heuristic
        ...
        self.k=2             # start at order‑2 to reduce micro‑steps
        self.equal=0
        self.LU=None
    ...
    def step(self):
        ...
        c=h/self.alp[k]
        # (re)factor only if c changed >1% or LU missing
        if self.LU is None or abs((c-self._c_last)/self._c_last) > 0.01:
