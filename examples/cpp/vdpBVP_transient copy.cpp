...
typedef struct {
  ...
  PetscReal    l10, l20, x0, v0;   /* Initial conditions for x1(0), x2(0) */
...
/* -------------------------------------------------------------------
   ...
   For the minimum time Hamiltonian
   H = l1*x2 + l2*(ustar * (1 - x1^2) * x2 - x1) + l3*1
     if l2*(1 - x1^2) * x2 < 0, then ustar = 1
     if l2*(1 - x1^2) * x2 > 0, then ustar = -1
     l1' = -2*x1*x2*l2*ustar
     l2' = l1+l2*ustar*(1-x1^2)
     l3' = 0
     x1' = x2
     x2' = ustar * (1 - x1^2) * x2 - x1
     x3' = 1 //This is the time derivative
...
static PetscErrorCode RHSFunction_Inner(TS ts, PetscReal t, Vec Y, Vec Ydot, void* ctx)
{
  ...
  if (l2*(1 - x1*x1) * x2 < 0) {
    ustar = 1;
    f[0] = -2*x1*x2*l2*ustar;
    f[1] = l1+l2*ustar*(1-x1*x1);
    f[2] = 0;
    f[3] = x2;
    f[4] = ustar * (1 - x1*x1) * x2 - x1;
    f[5] = 1;
  } else {
    ustar = -1;
    f[0] = -2*x1*x2*l2*ustar;
...
static PetscErrorCode RHSFunction_Outer(TS ts, PetscReal tau, Vec Tvec, Vec Tdot, void* ctx)
{
  ...
  /* 2) Reset the inner solution to initial conditions x(0)=(l10, l20, x0, v0). */
