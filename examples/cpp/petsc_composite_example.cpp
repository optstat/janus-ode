...
int main(int argc, char **argv)
{
  ...
  ierr = SNESSetType(snes, SNESCOMPOSITE); CHKERRQ(ierr);
  ...
  /* Add sub-SNES by type (new in PETSc 3.22+).
     For example, sub-SNES #0 is a Richardson approach */
  ierr = SNESCompositeAddSNES(snes, SNESNRICHARDSON); CHKERRQ(ierr);

  /* Then sub-SNES #1 is a Newton line-search approach */
  ierr = SNESCompositeAddSNES(snes, SNESNEWTONLS); CHKERRQ(ierr);


  /* Optionally set from command line, e.g. -snes_monitor */
  ierr = SNESSetFromOptions(snes); CHKERRQ(ierr);

  /* [Optional] If you want to customize each sub-SNES further,
     retrieve it by index:
         SNES subsnes0, subsnes1;
         SNESCompositeGetSNES(snes, 0, &subsnes0);
         SNESCompositeGetSNES(snes, 1, &subsnes1);
     Then call SNESSetTolerances(), SNESSetMaxLinearSolveIterations(), etc.
  */
