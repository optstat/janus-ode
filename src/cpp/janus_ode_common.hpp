...
{
    // Create tensors with gradient tracking for x and no gradient tracking for p
    auto xt = x.clone().detach_().set_requires_grad_(false);
    auto pt = p.clone().detach_().set_requires_grad_(true);
...
{
    // Create tensors with gradient tracking for x and no gradient tracking for p
    auto xt = x.clone().detach_().set_requires_grad_(true);
    auto pt = p.clone().detach_().set_requires_grad_(false);
