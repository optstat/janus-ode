...
while (Stat.StepNbr <= MaxNbrStep && PosNeg*t < PosNeg*tfinal) % ligne 849 fortran
  ...
  % BLOCK DEPENDENT ON STAGE NUMBER
  if Variab && ~Keep  
...
% ------- COMPUTE THE MATRICES E1 AND E2 AND THEIR DECOMPOSITIONS Q R L U P 
  if NeedNewLU                                           
    [U_Sing,QRs] = DecomRC(h,ValP,Mass,Jac,RealYN);
...
% ------- LOOP FOR THE SIMPLIFIED NEWTON ITERATION 
  ...
  countNewt = 0;
  while NewtContinue %m1_11
    countNewt = countNewt+1;

    Reject = false;
    Newt   = Newt + 1;
    if Newt > Nit              % GOTO 78 m1_11_1
      UnExpStepRej        = true; 
 ...
 %   m1_11_2_1 filters the right stage   
...
% ------- SOLVE THE LINEAR SYSTEMS    % Ligne 1037                    
    z                 = Solvrad(z,w,ValP,h,QRs,Mass,RealYN); 
    ...
    NewNrm = NewNrm/SqrtStgNy;   
    % DYNO
% ------- TEST FOR BAD CONVERGENCE OR NUMBER OF NEEDED ITERATIONS TO LARGE        1050	   
	if ( Newt > 1 && Newt < Nit )	   % Check for slow convergence m1_11_2_2
	    thq = NewNrm/OldNrm;
	    if ( Newt == 2 ) %m1_11_2_2_1
	      Theta = thq;
        else 
	      Theta = sqrt(thq*thqold);
        end      
	    thqold = thq;                                      % 1058	 
	    if Theta < 0.99        % Convergence at least m1_11_2_2_3      
	      FacConv = Theta/(1-Theta);
	      dyth    = FacConv*NewNrm*Theta^(Nit-1-Newt)/FNewt;        
	      if ( dyth >= 1 )  %m1_11_2_2_3_1
    ...
    w      = w+z;                % w  <-> FF,  z  <->  ZZ     
    if count == 4 && countNewt == 2
        stopHere = true;
    end

    for n = 1:Ny 
      z(n,:) = T(1,:) * w(n,1);
      for q = 2:NbrStg   %m1_11_3_1           
        z(n,:) = z(n,:) + T(q,:) * w(n,q); 
      end
    end   

	if (FacConv*NewNrm > FNewt) %m1_11_3_2
      continue                  % GOTO 40 il faut mettre continue en Matlab
    end    
    NewtContinue = false;
  end     % Newton ok, perform one step END OF Newton while internal loop
  ...
  [err,Stat] = Estrad(z,Dd,h,QRs,Mass,Scal,f0, ...
...
% ------- WE REQUIRE .2<=HNEW/H<=8. 1/FacL <= hnew/h <= 1/FacR      
  ...
  exponent = 1/NbrStg + 1;
  % Step 2: Compute err to the power of exponent
  err_powered = err^exponent;
  % Step 3: Apply the factor 'fac' to the powered err
  scaled_err = err_powered / fac;
  % Step 4: Ensure scaled_err is at least as large as FacL
  limited_err = min(FacL, scaled_err);
  % Step 5: Ensure limited_err does not exceed FacR
...
% ------- PREDICTIVE CONTROLLER OF GUSTAFSSON  
    ...
    %END OF DEPENDENCY ON STAGE NUMBER
    ...
    %THE FOLLOWING BLOCK IS INDEPENDENT OF THE NUMBER OF STAGES
...
function  [U_Sing, QRs] = DecomRC(h,ValP,Mass,Jac,RealYN)
valp   = ValP/h;
NbrStg = length(valp);
if RealYN 
  NuMax = (NbrStg+1)/2; 
  ...
  if NbrStg > 1
    for q = 1:(NbrStg-1)/2
      ...
      [row, col] = size(B);
      %fprintf('Size of the matrix B is %d rows by %d columns\n', row, col);
      %[LL,UU,PP] = lu(B);   
      %L(:,:,q1)  = full(LL);
      %U(:,:,q1)  = full(UU);
      %P(:,:,q1)  = full(PP);
...
function z = Solvrad(z,w,ValP,h,QRs,Mass,RealYN)
...
return
function [err,Stat] = Estrad(z,Dd,h,QRs,Mass,Scal,f0, ...
                                 First, Reject,Stat,...
                                 OdeFcn,t,y,varargin)  
