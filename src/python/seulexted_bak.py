...
class SeulexTedState:
    def __init__(self, yin, xin, paramsin, nparams_local, atoll, rtoll, KMAXX=13):
        assert isinstance(yin, torch.Tensor)
        assert isinstance(xin, torch.Tensor)
        self.nparams_local = nparams_local
        self.M, self.D = yin.shape
        self.N = self.D+nparams_local #This is the expanded space
        dualy = torch.eye(self.N, dtype=yin.dtype, device=yin.device).unsqueeze(0).repeat(self.M, 1, 1)
        realy = torch.zeros((self.M, self.N), dtype=yin.dtype, device=yin.device)
        realy[:, 0:self.D] = yin
        realy[:, self.D:self.N] = paramsin
        ...
        self.table = TensorMatDual.createZero(torch.zeros((self.M, self.IMAXX, self.N), dtype=y.r.dtype, device=y.r.device), y.d.shape[2])
        self.dfdy = TensorMatDual.createZero(torch.zeros((self.M, self.N, self.N), dtype=y.dtype, device=y.device), y.d.shape[2])
        ...
        self.one = TensorDual.createZero(torch.ones((self.M, 1), dtype=torch.float64, device=y.device), y.d.shape[2])
        self.nseq[:, 0] = 2
        self.nseq[:, 1] = 3
    ...
    def reset_init(self, paramsloc, ft):
        self.x.d *=0.0
        self.coeff.d *=0.0
        self.cost.d *=0.0
        self.costfunc.d *=0.0
        ...
        self.table.d*=0.0
        self.theta.d*=0.0
        self.y.d  =torch.eye(self.N, dtype=self.y.r.dtype, device=self.y.r.device).unsqueeze(0).repeat(self.M, 1, 1)
        self.y.r[:, self.D:self.N-1] = paramsloc
        self.y.r[:, -1] = ft.r #Set the final time for completeness in the state.  This is a special parameter
...
class SeulexTed:
    ...
    def step(self, htry, dyns, ftin, state : SeulexTedState)->SeulexTedState:
        ...
        errold = TensorDual.createZero(torch.zeros((state.M), dtype=state.y.r.dtype, device=state.y.r.device), state.y.d.shape[2])
                ...
                while (m2[m2] & (k[m2]  <= (state.k_targ[m2]+1)) & bmask[m2]).any():
                    ...
                    if m21.any():
                            ...
                            def test_dy_dual():
                                ...
                                successc, thetaresc, yseqc = self.dy_dual(ysavtempc, xtempc, hnewtempc, km21c, \
                                                                            nseqm21,  scalem21c, \
                                ...
                                for i in range(NN):
                                    ...
                                    successc2p, thetaresc2p, yseqc2p = self.dy_dual(ysavtempc2p, xtempc2p, hnewtempc2p, km21c, \
                                                                              nseqm21, scalem21c, \
                                    ...
                                    successcp, thetarescp, yseqcp = self.dy_dual(ysavtempcp, xtempcp, hnewtempcp, km21c, \
                                                                              nseqm21, scalem21c, \
                                    ...
                                    successcm, thetarescm, yseqcm = self.dy_dual(ysavtempcm, xtempcm, hnewtempcm, km21c, \
                                                                                nseqm21, scalem21c, \
                                    ...
                                    successc2m, thetaresc2m, yseqc2m = self.dy_dual(ysavtempc2m, xtempc2m, hnewtempc2m, km21c, \
                                                                                nseqm21, scalem21c, \
                            ...
                            if m213.any():
                                ...
                                sq_inpt = delta*scalem21.reciprocal()
                                ...
                                t1 = ((TensorDual.ones_like(errm213) * state.EPS.reciprocal()))
                                j2 = errm213
                                # m2131 = m213 & bmask
                                m2131 = m213 & bmask
                                km213 = k[m213]
                                ...
                                if m213.any():
                                    ...
                                    if m2136.any():
                                        ...
                                        if m2136.any():
                                            ...
                                            temp = state.nseq.gather(1,nseq_indcs) * 2.0
    ...
    def dy_dual(self, y, x, htot, k, nseq, scale, thetain, dfdy, dyns):
        #print(f'k={k}')
        theta = thetain.clone()
